...
def createLayer(x, szW, trainning,lastLayer):
    ...
    """
    W=tf.compat.v1.get_variable('W',shape=szW,initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
...
class Aclass:
    ...
    def __init__(self, csm,mask,lam):
        with tf.compat.v1.name_scope('Ainit'):
            s=tf.shape(input=mask)
            ...
            #self.tol=tol
    def myAtA(self,img):
        with tf.compat.v1.name_scope('AtA'):
            ...
            coilComb= tf.reduce_sum(input_tensor=coilImgs*tf.math.conj(self.csm),axis=0)
...
def myCG(A,rhs):
    ...
    def body(i,rTr,x,r,p):
        with tf.compat.v1.name_scope('cgBody'):
            Ap=A.myAtA(p)
            alpha = rTr / tf.cast(tf.reduce_sum(input_tensor=tf.math.conj(p)*Ap), dtype=tf.float32)
            alpha=tf.complex(alpha,0.)
            x = x + alpha * p
            r = r - alpha * Ap
            rTrNew = tf.cast( tf.reduce_sum(input_tensor=tf.math.conj(r)*r), dtype=tf.float32)
            beta = rTrNew / rTr
    ...
    x=tf.zeros_like(rhs)
    i,r,p=0,rhs,rhs
    rTr = tf.cast( tf.reduce_sum(input_tensor=tf.math.conj(r)*r), dtype=tf.float32,)
    loopVar=i,rTr,x,r,p
    out=tf.while_loop(cond=cond,body=body,loop_vars=loopVar,name='CGwhile',parallel_iterations=1)[2]
    return c2r(out)
...
def makeModel(atb,csm,mask,training,nLayers,K,gradientMethod):
    ...
    out={}
    out['dc0']=atb
    with tf.compat.v1.name_scope('myModel'):
